...
class RecoverPassword extends React.Component {
    ...
    componentDidMount() {
        ...
        axios.get(backendUrl + `/reset/toUser/${token}`)
            .then(res => {
    ...
    onButtonPressed = () => {
            axios.post(backendUrl + '/save/reseted', {
                password: this.state.form.password.value,
                resetLink: this.state.token
            })
                .then(res => {
                    this.props.history.push(this.props.match.path.slice(0, 3) + '/log-in?changed-password=true');
                    console.log(res);
                })
                .catch(err => {
